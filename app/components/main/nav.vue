<template>
    <div>
        <van-tabs v-model:active="active" @change="render" swipeable >
            <van-tab title="推荐">
                <div>
            ...
            <van-tab :title="item.id" :key="item.id" v-for="item in categorieslist">
                <template #title> 
...
const getArticleList = async () => {
    const { error, data } = await useSupabaseClient().from('articles').select().order('order',{ascending:false})
    if (!error) {
        return data
    } else {
        return []
...
const render = async (index,title) => {
    ...
    categoriesId.value = title
    // 重新获取数据
    if (categoriesId.value!=='推荐') {
        showList.value = await useSupabaseClient().from('articles').select().eq('category_id', categoriesId.value).order('order',{ascending:false}).then(res => {
            if (!res.error) {
                return res.data
            } else {
                return []
